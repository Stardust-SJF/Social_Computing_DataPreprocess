    ...
    if member_id in member_group.keys():
        if group_id in member_group[member_id][0]:
            if join_time < member_group[member_id][1][member_group[member_id][0].index(group_id)]:
                member_group[member_id][1][member_group[member_id][0]] = join_time
            else:
                return
        else:
...
def data_preprocess():
    ...
    # data_group = pd.read_csv("./Data/group.csv", error_bad_lines=False ) # id, name, urlname, rating, categoryname, join_mode, lat, lon, country, city, created, members
    ...
    for index, row in data_events.iterrows():
        event_group[(row['id'])] = int(row['groupid'].replace('"', '').replace(',', ''))

    # event host is in group
    for index, row in data_event_host.iterrows():
        member_group_add((row['memberid']), event_group[(row['eventid'])], -1)
